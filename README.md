...
[![NPM Version](https://img.shields.io/npm/v/%40sendevia%2Fpi-ask-author?style=flat-square)](https://www.npmjs.com/package/@sendevia/pi-ask-author)
...
- **小说级富文本与草稿高亮**
  - 内置紧凑轻量的终端 Markdown 渲染与小说语法高亮，完整支持粗体、斜体、删除线、行内代码、引用区块。
  - 对中文小说台词（引号内对话高亮）与分段排版做了专门处理，作者可以在终端内直接检视行文节奏与语感。
- **题目与选项两级批注体系**
  - **题目级补充说明**：对该题整体提出全局创作指令或氛围补充。
  - **选项专属补充说明**：光标位于选项行时按 `n` 即可为特定选项追加细化批注（**未勾选的选项同样可以填写补充说明**，便于模型借鉴未选方案中的可取之处）。
- **单题平铺与批量问卷双模式**
...
# 或从 npm 安装
pi install npm:@sendevia/pi-ask-author
```
...
| 按键                                  | 功能说明                                                                     |
| :------------------------------------ | :--------------------------------------------------------------------------- |
| `Tab` `Shift+Tab` / `←` `→` / `h` `l` | 在题目页与提交总览页之间循环切换                                             |
| `1`~`9`                               | 直接跳转到第 N 题                                                            |
| `↑` `↓` / `k` `j`                     | 作答页移动菜单光标；总览页上下滚动                                           |
