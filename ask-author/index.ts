 ...
 *
 * 核心架构特性：
 * - 四类出口（取消 / 结构化报错 / 运行期异常 / 正常完成）由四个结果工厂统一产出；其中取消、结构化报错与运行期异常三类共享 `ResultContext` 的缺省收敛，完成结果由表单模型直接供给
...
import { cleanOptional, safeKeyHint } from "./format.js";
...
/** `ask_author` 工具执行结果契约（基于 Pi 官方 `AgentToolResult` 结构特化） */
...
/**
 * 构造运行期异常结果（模型构建抛错与 `execute` 异常捕获两条路径共用本工厂）
 ...
 * 校验项：普通对象、`cancelled`/`answers`/`questions`/`formTitle` 键存在性、`cancelled` 布尔、`answers`/`questions` 数组、`formTitle` 字符串，任一不满足即返回 undefined，交由渲染器走取消/兜底卡片路径
 * 可选 `error` / `formDescription` 不设闸门：二者仅参与字符串插值，非字符串至多渲染为退化文本（`fg` 为模板拼接，不抛错）
 * @param value - 宿主传入的不可信 `details` 值
 * @returns 通过形状闸门的 `AskAuthorResult`；任一校验项失败返回 undefined
 */
function narrowResultDetails(value: unknown): AskAuthorResult | undefined {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return undefined;
  if (!("cancelled" in value) || !("answers" in value) || !("questions" in value) || !("formTitle" in value)) {
    return undefined;
  }
  if (typeof value.cancelled !== "boolean") return undefined;
  if (!Array.isArray(value.answers) || !Array.isArray(value.questions)) return undefined;
  if (typeof value.formTitle !== "string") return undefined;
...
export interface CallCardData {
  /** 卡片标题（已清洗的 `formTitle`；构建失败时为缺省标题） */
  title: string;
  /** 是否为兜底卡片（不渲染计数后缀、标题不加粗） */
  fallback: boolean;
  /** 计数标签（多题模式为 `2 题 • 6 选项`，单题模式仅 `6 选项`；兜底卡片为空串） */
  countLabel: string;
}
...
/**
 * 派生调用卡片数据（标题 + 计数标签：题目数 > 1 时附带题量，单题模式仅选项数；清洗或建模型抛错时返回兜底数据）
...
function resolveCallCard(args: RawParams, state: CallRenderState): CallCardData {
  ...
  try {
    ...
    card = {
      title: model.formTitle || TEXTS.fallbacks.defaultFormTitle,
      fallback: false,
      countLabel:
    ...
    card = { title: TEXTS.fallbacks.defaultFormTitle, fallback: true, countLabel: "" };
  }
...
export default function askAuthorExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool<typeof AskAuthorParams, AskAuthorResult, CallRenderState>({
    ...
    // 宿主契约要求可变 `string[]`；TEXTS 为 `as const` 只读元组，故展开构造可变副本
     ...
     * 守卫顺序：信号已中止 → 取消；非 TUI 模式 → 非交互报错；模型构建抛错 → 运行期异常；无有效题目 → 空题目报错；`ui.custom` 回传结果映射为取消 / 完成；`finally` 兜底 `dispose()`
    ...
    async execute(_toolCallId, rawParams, signal, _onUpdate, ctx) {
      ...
      try {
        // 宿主回调契约 `(tui, theme, keybindings, done)`：按键分派由组件内部 `matchesKey` 自持，
        ...
        // dispose() 幂等，宿主亦会卸载组件；此处仅兜住「组件工厂执行后、宿主接管前」的抛错路径
        comp?.dispose();
      }
    },

    /**
     * 会话回放调用卡片渲染（复用槽位既有 Text；派生按 args 身份记忆化，失败时返回仅含标题的兜底卡片）
     * `args` 由宿主原样透传（流式期间可能不完整，且未经 Schema 校验；此处 `RawParams` 类型只是宿主泛型的静态断言），故必须先经 `sanitizeAskAuthorArgs` 收敛再建模型
    ...
    renderCall(args, theme, context) {
      ...
      const body = card.fallback
        ? card.title
        : `${theme.bold(card.title)} ${theme.fg("muted", `(${card.countLabel})`)}`;
      return renderCardText(`${label} ${body}`, context.lastComponent);
    },

    /**
     * 会话回放结果卡片渲染（错误 / 取消 / 完成三态，展开态追加答卷明细；分支与兜底恒返回 Text）
     ...
     * @returns 结果卡片 `Text`（复用 `context.lastComponent` 就地更新；渲染分支抛错时退化为状态徽标）
     */
    renderResult(result, { expanded }, theme, context) {
      ...
      try {
        if (details?.error) {
          ...
          if (!expanded) {
            return renderCardText(
              `${errorTitle} (${safeKeyHint("app.tools.expand", TEXTS.card.expandHint)})`,
        ...
        if (!expanded) {
          return renderCardText(
            `${countLabel} (${safeKeyHint("app.tools.expand", TEXTS.card.expandHint)})`,
            context.lastComponent,
          );
        }
        ...
        // 渲染兜底：仅呈现状态徽标，避免影响会话主流程
        const fallback = details?.error
          ? theme.fg("error", TEXTS.card.resultErrorShort)
          : details?.cancelled
            ? theme.fg("warning", TEXTS.card.resultCancelled)
            : theme.fg("success", TEXTS.card.resultCompleted);
        return renderCardText(fallback, context.lastComponent);
      }
    },
  });
