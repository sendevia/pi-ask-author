 ...
 * - 默认值不写进 Schema：题目内字段由 `sanitize.ts` 规整时填入（`multiSelect`→false、`allowCustom`→true），模型层再按同一语义兜底
...
const OptionSchema = Type.Object({
  /** 选项简短标题/行动标签（必填；缺失时经 `sanitize.ts` → `model.ts` 两级兜底为自动占位标签） */
...
const QuestionSchema = Type.Object({
  ...
  /** 候选选项列表（必填；1 项起、上限 `MAX_OPTIONS`，建议 2~10 项） */
...
/** `ask_author` 工具完整参数 Schema（必填 `formTitle`；批量问卷传 `questions`，单题平铺传顶层 `question` + `options`） */
export const AskAuthorParams = Type.Object({
  /** 问卷总任务标题（必填；简明概括请示主题） */
  formTitle: Type.String({ description: TEXTS.schema.formTitle }),
  /** 问卷全局背景或前置情境（可选；平铺单题模式下同时充当该题描述） */
